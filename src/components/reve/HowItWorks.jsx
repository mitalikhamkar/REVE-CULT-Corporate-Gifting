...
export default function HowItWorks() {
  return (
    <section className="py-20 lg:py-32 px-5 sm:px-8 lg:px-14">
      ...
      >
        <div aria-hidden="true" className="hidden lg:block absolute top-6 left-[12%] right-[12%] h-px bg-reve-terracotta/30" />
        {PROCESS_STEPS.map((s) => (
          <motion.li key={s.number} variants={fadeUp} className="relative">
            ...
            <p className="mt-2.5 text-sm text-reve-brown leading-relaxed max-w-xs">{s.text}</p>
          </motion.li>
        ))}
      </motion.ol>
    </section>
